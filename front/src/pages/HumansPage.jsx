...
export default function HumansPage() {
  ...
  const fetchPage = React.useCallback(
      async (p = page, s = size, f = filters, so = sort) => {
        ...
        setError("");
        try {
          const resp = await HumansApi.list({
            page: p,
            size: s,
            sort: so,
            filters: f,
          ...
          if (resp.totalPages > 0 && p >= resp.totalPages) {
            ...
            const retry = await HumansApi.list({
              page: last,
              size: s,
              sort: so,
              filters: f,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const cityIds = Array.isArray(cityIdsRaw) ? cityIdsRaw : [];
      const usage =
      ...
      const count = usage !== null ? usage : cityIds.length;

      const message =
          count && cityIds.length
              ? `Человек id=${id} связан с ${count} городами: [${cityIds.join(
                  ", "
              )}]. Перепривяжите эти города к другому человеку и повторите удаление.`
              : "Невозможно удалить: человек связан с городами. Перепривяжите города к другому человеку и повторите удаление.";

      setLinkInfo({ message, usage: count, cityIds, humanId: id });
    }
  };
  ...
  const hasLinkUsage =
      linkInfo &&
      ((Number.isFinite(linkInfo.usage) && linkInfo.usage > 0) ||
  ...
  return (
      <div style={{ padding: 24, maxWidth: 900, margin: "0 auto" }}>
        ...
        >
          ...
          <button onClick={() => setCreateOpen(true)} style={createBtn}>
            Создать человека
          </button>
          ...
          >
            ...
            <input
                ...
                onChange={(e) =>
                    setFilters((f) => ({ ...f, height: e.target.value }))
                }
                style={input}
                title="Точный фильтр по росту (height)"
            />
            <button onClick={applyFilters} style={btn}>
              Применить
            </button>
            <button onClick={resetFilters} style={btnLight}>
              Сброс
            </button>
          </div>
          ...
          <span style={{ opacity: 0.7, fontSize: 13 }}>
            ...
            <button
                onClick={() => fetchPage(page, size, filters, sort)}
                disabled={loading}
            >
              Обновить
            </button>
        ...
        >
          <table style={table}>
            ...
            <tr>
              ...
              <th style={{ ...th, width: 220, textAlign: "right" }}>
                Действия
              </th>
            </tr>
            </thead>
                ...
                <tr key={r.id} style={i % 2 ? trZebra : undefined}>
                  ...
                  <td style={{ ...td, textAlign: "right" }}>
                    <div style={actionsWrap}>
                      <button style={btn} onClick={() => handleEditClick(r)}>
                        Изменить
                      </button>
                      <button
                          style={btnDanger}
              ...
              <div style={{ display: "grid", gap: 8 }}>
                ...
                {hasLinkUsage && (
                    <div style={{ marginTop: 4, fontSize: 13 }}>
                      Перепривяжите эти города к другому человеку и повторите
                      удаление.
                    </div>
                )}
              </div>
