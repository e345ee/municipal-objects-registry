...
import CoordinatesCreateDialog from "./CoordinatesCreateDialog";
import CoordinatesEditDialog from "./CoordinatesEditDialog";
...
export default function CoordinatesPage() {
  ...
  const fetchPage = React.useCallback(
    async (p = page, s = size, f = filters, so = sort) => {
      ...
      setError("");
      try {
        const resp = await CoordinatesApi.list({ page: p, size: s, sort: so, filters: f });
        if (resp.totalPages > 0 && p >= resp.totalPages) {
          const last = resp.totalPages - 1;
          const retry = await CoordinatesApi.list({ page: last, size: s, sort: so, filters: f });
  ...
  React.useEffect(() => {
    const client = createStomp(() => { fetchPage(page, size, filters, sort); }, ["/topic/coordinates"]);
    client.activate();
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const message =
        resp?.message ??
        "Невозможно удалить: координаты используются в городах. Перепривяжите города к другим координатам.";

      setLinkInfo({
        message,
        usage,
        cityIds: Array.isArray(cityIds) ? cityIds : null,
        coordId: id,
      });
    }
  };
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100, margin: "0 auto" }}>
      <header style={{ display: "flex", gap: 12, marginBottom: 12, alignItems: "center", flexWrap: "wrap" }}>
        <h2 style={{ margin: 0 }}>Координаты</h2>
        <button onClick={() => setCreateOpen(true)} style={createBtn}>Создать координаты</button>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ opacity: 0.7, fontSize: 13 }}>Показаны {from}–{to} из {totalElements}</span>
      ...
      <div style={{ overflowX: "auto", border: "1px solid #e5e7eb", borderRadius: 8 }}>
        <table style={table}>
          <thead>
            <tr>
              <th
                style={{...th, width: 90, cursor: "pointer", userSelect: "none"}}
              ...
              <th style={{...th, width: 200, textAlign: "right"}}>Действия</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.id} style={i % 2 ? trZebra : undefined}>
                ...
                <td style={{...td, textAlign: "right"}}>{fmt(r.x)}</td>
                <td style={{...td, textAlign: "right"}}>{fmt(r.y)}</td>
                <td style={{...td, textAlign: "right"}}>
                  <div style={actionsWrap}>
                    <button style={btn} onClick={() => onEditClick(r)}>Изменить</button>
            ...
            {!loading && rows.length === 0 && (
              <tr><td style={tdEmpty} colSpan={4}>Пусто</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <Modal
        ...
        title="Нельзя удалить координаты"
        ...
        {linkInfo && (
          <div style={{ display: "grid", gap: 8 }}>
            ...
            <div style={{ marginTop: 4, fontSize: 13 }}>
              Перепривяжите эти города к другим координатам и повторите удаление.
            </div>
          </div>
        )}
      </Modal>
